...
- For endpoints that use a key/value msgpack stream, add `"framing": "kv-stream"` inside `blob1` in
  your `decoded.json`. This is the only switch the builder uses; there is no path-based detection.
