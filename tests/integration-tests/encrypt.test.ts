...
describe('encrypt build e2e', () => {
  test('builds request from decoded.json and can decrypt blob2 back', async () => {
    ...
    try {
      ...
      const obj = unpackLengthPrefixedMsgpack(plaintext) as any;
      expect(obj).toEqual({ answer: 42, s: 'hi' });
    } finally {
      // cleanup
      fs.rmSync(path.join('encrypt', 'input', '__test__'), { recursive: true, force: true });
      fs.rmSync(path.join('encrypt', 'output', '__test__'), { recursive: true, force: true });
    }
  });
});
